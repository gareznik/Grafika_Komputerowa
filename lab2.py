...
def create_linspace(start, stop, num):
    """
    Создает список из 'num' чисел в диапазоне от 'start' до 'stop'.
    Это замена для numpy.linspace().
    """
    if num == 1:
        return [start]
...
def calculate_egg_vertices():
    """
    Рассчитывает и сохраняет 3D-координаты вершин яйца в список 'vertices'.
...
def draw_egg_points():
    """
    Отрисовывает модель яйца с помощью точек (GL_POINTS),
    используя ранее рассчитанные вершины.
    """
